...
*/
static void vlogSignature(unsigned char *p, int n, char *zCksum, int nCksum){
  unsigned int s0 = 0, s1 = 0;
  unsigned int *pI;
  int i;
  if( n<=16 ){
    for(i=0; i<n; i++) snprintf(zCksum+i*2, nCksum-i*2, "%02x", p[i]);
  }else{ 
    pI = (unsigned int*)p;
    for(i=0; i<n-7; i+=8){
      s0 += pI[0] + s1;
    ...
    for(i=0; i<8; i++) snprintf(zCksum+i*2, nCksum-i*2, "%02x", p[i]);
    snprintf(zCksum+i*2, nCksum-i*2, "-%08x%08x", s0, s1);
  }
}
