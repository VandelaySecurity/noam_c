...
static i64 describeContent(
  ...
  while( i>0 && pData<=pLimit ){
    ...
    zDesc++;
    if( x==0 ){
      snprintf(zDesc, nLocal, "*");     /* NULL is a "*" */
    }else if( x>=1 && x<=6 ){
      ...
      snprintf(zDesc, nLocal, "%lld", v);
    }else if( x==7 ){
      snprintf(zDesc, nLocal, "real");
      pData += 8;
    }else if( x==8 ){
      snprintf(zDesc, nLocal, "0");
    }else if( x==9 ){
      snprintf(zDesc, nLocal, "1");
    }else if( x>=12 ){
      i64 size = (x-12)/2;
      if( (x&1)==0 ){
        snprintf(zDesc, nLocal, "blob(%lld)", size);
      }else{
        snprintf(zDesc, nLocal, "txt(%lld)", size);
      }
      pData += size;
    }
    j = (int)strlen(zDesc);
