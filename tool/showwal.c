...
static i64 describeContent(
  ...
  while( i>0 && pData<=pLimit ){
    ...
    zDesc++;
    if( x==0 ){
      snprintf(zDesc, remainingSpace, "*");     /* NULL is a "*" */
    }else if( x>=1 && x<=6 ){
      ...
      snprintf(zDesc, remainingSpace, "%lld", v);
    }else if( x==7 ){
      snprintf(zDesc, remainingSpace, "real");
      pData += 8;
    }else if( x==8 ){
      snprintf(zDesc, remainingSpace, "0");
    }else if( x==9 ){
      snprintf(zDesc, remainingSpace, "1");
    }else if( x>=12 ){
      i64 size = (x-12)/2;
      if( (x&1)==0 ){
        snprintf(zDesc, remainingSpace, "blob(%lld)", size);
      }else{
        snprintf(zDesc, remainingSpace, "txt(%lld)", size);
      }
      pData += size;
    }
    j = (int)strlen(zDesc);
    if( j >= remainingSpace ){
      break;  /* Insufficient space, stop processing */
    }
    zDesc += j;
    nDesc += j;
    remainingSpace -= j;
