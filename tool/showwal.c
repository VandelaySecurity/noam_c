...
static i64 describeContent(
  ...
  while( i>0 && pData<=pLimit ){
    ...
    zDesc++;
    if( x==0 ){
      snprintf(zDesc, nDesc2 - nDesc, "*");     /* NULL is a "*" */
    }else if( x>=1 && x<=6 ){
      ...
      snprintf(zDesc, nDesc2 - nDesc, "%lld", v);
    }else if( x==7 ){
      snprintf(zDesc, nDesc2 - nDesc, "real");
      pData += 8;
    }else if( x==8 ){
      snprintf(zDesc, nDesc2 - nDesc, "0");
    }else if( x==9 ){
      snprintf(zDesc, nDesc2 - nDesc, "1");
    }else if( x>=12 ){
      i64 size = (x-12)/2;
      if( (x&1)==0 ){
        snprintf(zDesc, nDesc2 - nDesc, "blob(%lld)", size);
      }else{
        snprintf(zDesc, nDesc2 - nDesc, "txt(%lld)", size);
      }
      pData += size;
    }
    j = (int)strlen(zDesc);
